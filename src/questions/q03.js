...
export function faturamento() {
  ...
  const menorFaturamento = Math.min(...faturamentoComValores.map(dia => dia.valor));
  const maiorFaturamento = Math.max(...faturamentoComValores.map(dia => dia.valor));

  const somaFaturamento = faturamentoComValores.reduce((total, dia) => total + dia.valor, 0);
